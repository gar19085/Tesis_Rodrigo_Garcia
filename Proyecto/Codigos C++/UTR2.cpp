...
void RTU2(int sockfd, sockaddr_in& addr, socklen_t& length); // Declaración de la función RTU2
...
void agregar_evento(uint8_t evento_id) { // Función para agregar eventos
    ...
    // Formato del mensaje
    std::sprintf(message, "RTU2 %d %s %d%d %d%d %d %d%d %0.2f",
             evento_id, timestamp, 
...
int main(int argc, char *argv[]) { // Función principal
   ...
   if(argc != 2) // Verifica si se ingresó el puerto
    ...
    std::thread rtuThread(RTU2, sockfd, std::ref(addr), std::ref(length)); // Crea el hilo de la función RTU2
...
void RTU2(int sock, sockaddr_in& addres, socklen_t& leng){ // Función para la RTU2
    LED luz1(LUZ_1);
    LED luz2(LUZ_2);
    ...
    while (1){ 
        ...
        if((std::strcmp(buffer, "RTU2 LED1 1") == 0)){ // Verifica si el mensaje es para encender el LED 1
            char mensaje[60];
            luz1.on();
            LD1 = 1;
            agregar_evento(9); // Agrega el evento 9
        }
        if((std::strcmp(buffer, "RTU2 LED1 0") == 0)){ // Verifica si el mensaje es para apagar el LED 1
            char mensaje[60];
            luz1.off();
            LD1 = 0;
            agregar_evento(10);
        }
        if((std::strcmp(buffer, "RTU2 LED2 1") == 0)){ // Verifica si el mensaje es para encender el LED 2
            char mensaje[60];
            luz2.on();
            LD2 = 1;
            agregar_evento(11);
        }
        if((std::strcmp(buffer, "RTU2 LED2 0") == 0)){ // Verifica si el mensaje es para apagar el LED 2
            char mensaje[60];
            luz2.off();
            LD2 = 0;
            agregar_evento(12);
        }
        if((std::strcmp(buffer, "RTU2 LEDIoT 1") == 0)){ // Verifica si el mensaje es para encender el LED IoT
            char mensaje[60];
            char data1[] = "A"; 
            status_led_1 = 1;
            agregar_evento(13);
        ...
        if((std::strcmp(buffer, "RTU2 LEDIoT 0") == 0)){ // Verifica si el mensaje es para apagar el LED IoT
            char mensaje[60]; 
            char data0[] = "B";
            status_led_1 = 0;
            agregar_evento(14);
