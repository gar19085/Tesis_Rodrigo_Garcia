...
void RTU2(int sockfd, sockaddr_in& addr, socklen_t& length);
...
void agregar_evento(uint8_t evento_id) {
    ...
    std::sprintf(message, "RTU2 %d %s %d %d%d %d%d %d %d%d %0.2f",
...
int main(int argc, char *argv[]) {
   ...
   if(argc != 2)
    ...
    std::thread rtuThread(RTU2, sockfd, std::ref(addr), std::ref(length));
...
void RTU2(int sock, sockaddr_in& addres, socklen_t& leng){
    LED luz1(LUZ_1);
    LED luz2(LUZ_2);
    ...
    while (1){
        ...
        if((std::strcmp(buffer, "RTU2 LED1 1") == 0)){
            char mensaje[60];
            luz1.on();
            LD1 = 1;
            agregar_evento(9);
        ...
        if((std::strcmp(buffer, "RTU2 LED1 0") == 0)){
            char mensaje[60];
            luz1.off();
            LD1 = 0;
            agregar_evento(10);
        ...
        if((std::strcmp(buffer, "RTU2 LED2 1") == 0)){
            char mensaje[60];
            luz2.on();
            LD2 = 1;
            agregar_evento(11);
        ...
        if((std::strcmp(buffer, "RTU2 LED2 0") == 0)){
            char mensaje[60];
            luz2.off();
            LD2 = 0;
            agregar_evento(12);
        ...
        if((std::strcmp(buffer, "RTU2 LEDIoT 1") == 0)){
            char mensaje[60];
            char data1[] = "A";
            status_led_1 = 1;
            agregar_evento(13);
        ...
        if((std::strcmp(buffer, "RTU2 LEDIoT 0") == 0)){
            char mensaje[60];
            char data0[] = "B";
            status_led_1 = 0;
            agregar_evento(14);
