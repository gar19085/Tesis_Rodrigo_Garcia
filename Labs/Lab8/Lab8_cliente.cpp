...
void *receiving(void *ptr)
{
    ...
    while (true)
    {
        ...
        n = recvfrom(*sock, buffer, MSG_SIZE, 0, reinterpret_cast<struct sockaddr *>(&from), &length);
        if (n < 0)
...
int main(int argc, char *argv[])
{
    ...
    pthread_create(&thread_rec, nullptr, receiving, reinterpret_cast<void *>(&sock));
