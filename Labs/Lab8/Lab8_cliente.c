/* 	Nombre     : 	Lab8_cliente.c
	Autor      : 	Luis A. Rivera
	Descripción: 	IE3059 lab
					Lab 8. Sockets: Voto Master/Slave. CLIENTE				*/
...
#if OPCION_IP == 0
// --- hard coded -------------------------------------------------------------
	strcpy(IP_broadcast, "192.168.1.255");  // Cambiar, de ser necesario.
#endif
...
// ----------------------------------------------------------------------------
	...
	printf("También transmite lo que el usuario ingrese, máx. %d caracteres. (! para salir):\n",
	       MSG_SIZE);
	do
	{
		memset(buffer, 0, MSG_SIZE);	// "limpia" el buffer
